...
func (g *Generation) partitionWatcher(interval time.Duration, topic string, numPartitions int) {
	g.Start(func(ctx context.Context) {
		g.log(func(l Logger) {
...
func NewConsumerGroup(config ConsumerGroupConfig) (*ConsumerGroup, error) {
	...
	cg := &ConsumerGroup{
		config:       config,
		numPartition: make(map[string]int, len(config.Topics)),
...
type ConsumerGroup struct {
	...
	// Defines partitions number for each topic in config
	numPartition map[string]int
...
func (cg *ConsumerGroup) nextGeneration(memberID string) (string, error) {
	...
	if cg.config.WatchPartitionChanges {
		for topic, numPartitions := range cg.numPartition {
			gen.partitionWatcher(cg.config.PartitionWatchInterval, topic, numPartitions)
		}
	}
...
func (cg *ConsumerGroup) assignTopicPartitions(conn coordinator, group joinGroupResponseV1) (GroupMemberAssignments, error) {
	...
	// Resetting old states of topics partitions num
	for topic := range cg.numPartition {
		cg.numPartition[topic] = 0
	}

	// Setting new states of topics partitions num
	for _, partition := range partitions {
		cg.numPartition[partition.Topic] += 1
	}
